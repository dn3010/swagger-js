...
var debug = require('debug')('swagger:client');
var _ = {
...
var SuperagentHttpClient = function (opts) {
  debug('new SuperagentHttpClient');
 ...
 */
var SwaggerHttp = module.exports = function (options) {
  debug('new SwaggerHttp')
...
SuperagentHttpClient.prototype.execute = function (obj) {
  ...
  // set the http agent
  var proto = url.parse(obj.url).protocol;
  proto = proto.substring(0, proto.length - 1); // drop the trailing ':'
  if (r.agent){// Cannot set agent in a browser context
    if (proto === 'https' && typeof this._options.httpsAgent !== 'undefined') {
      debug('using custom https agent');
      r.agent(this._options.httpsAgent);
    } else if (proto === 'http' && typeof this._options.httpAgent !== 'undefined') {
      debug('using custom http agent');
      r.agent(this._options.httpAgent);
    }
  }
